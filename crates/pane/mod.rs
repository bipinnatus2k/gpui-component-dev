...
use gpui::{div, AnyElement, App, Bounds, EventEmitter, Pixels, SharedString, Window, Styled, ParentElement, IntoElement, Context};
use gpui::Fill::Color;
...
impl TabContent for PlaceholderTab {
    fn render(&self, _focused: bool, _window: &mut Window, cx: &mut App) -> AnyElement {
        div()
            .p_4()
            .child(Label::new("Empty tab").bg(cx.theme().muted))
            .into_any_element()
    }
